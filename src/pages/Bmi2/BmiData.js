const list1a = [
    "Breakfast (8:00-8:30AM): 3 egg whites + 1 toasted brown bread + 1/2 cup low fat milk (no sugar)",
    "Mid-Meal (11:00-11:30AM): 1 cup papaya",
    "Lunch (2:00-2:30PM): 1 cup arhar dal + 1 chapatti + 1/2 cup low fat curd + salad",
    "Evening (4:00-4:30PM): 1 cup vegetable soup",
    "Dinner (8:00-8:30PM): 1 cup pumpkin + 1 chapatti + salad",
]

const list1b = [
    "Breakfast (8:00-8:30AM): 1 onion stuffed chapatti + 1/2 cup low fat curd",
    "Mid-Meal (11:00-11:30AM): 1 cup coconut water",
    "Lunch (2:00-2:30PM): 1 cup moong dal/ chicken curry + 1 chapatti + salad",
    "Evening (4:00-4:30PM): 1 cup pomegranate",
    "Dinner (8:00-8:30PM): 1 cup beans + 1 chapatti + salad",
]

const list1c =[
    "Breakfast (8:00-8:30AM): 2 besan cheela + 1/2 cup low fat curd",
    "Mid-Meal (11:00-11:30AM): 1 apple",
    "Lunch (2:00-2:30PM): 1 cup masoor dal + 1 chapatti + 1/2 cup low fat curd + salad",
    "Evening (4:00-4:30PM): 1 cup tomato soup",
    "Dinner (8:00-8:30PM): 1 cup carrot peas vegetable +1 chapatti + salad",
]

const list1d =[
    "Breakfast (8:00-8:30AM): 1 cup vegetable brown bread upma + 1/2 cup low fat milk (no sugar)",
    "Mid-Meal (11:00-11:30AM): 1 cup musk melon",
    "Lunch (2:00-2:30PM): 1 cup rajma curry + 1 chapatti + salad",
    "Evening (4:00-4:30PM): 1 cup vegetable soup",
    "Dinner (8:00-8:30PM): 1 cup parwal vegetable + 1 chapatti + salad",
]

const list1e = [
    "Breakfast (8:00-8:30AM): 1 cucumber hung curd sandwich + 1/2 tsp green chutney + 1 orange",
    "Mid-Meal (11:00-11:30AM): 1 cup buttermilk",
    "Lunch (2:00-2:30PM): 1 cup white chana/ fish curry + 1 chapatti + salad",
    "Evening (4:00-4:30PM): 1 cup low fat milk (no sugar)",
    "Dinner (8:00-8:30PM): 1 cup cauliflower vegetable + 1 chapatti + salad",
]

const list1f = [
    "Breakfast (8:00-8:30AM)\t1 cup vegetable poha + 1 cup low fat curd",
    "Mid-Meal (11:00-11:30AM)\t1 cup watermelon",
    "Lunch (2:00-2:30PM)\t1 cup chana dal + 1 chapatti + salad",
    "Evening (4:00-4:30PM)\t1 cup sprouts salad",
    "Dinner (8:00-8:30PM)\t1 cup tinda vegetable + 1 chapatti + salad",

]

const list1g = [
    "Breakfast (8:00-8:30AM)\t1 cup low fat milk with oats + 3-4 strawberries",
    "Mid-Meal (11:00-11:30AM)\t1 cup coconut water",
    "Lunch (2:00-2:30PM)\t1 cup soybean curry + 1 chapatti + 1/2 cup low fat curd + salad",
    "Evening (4:00-4:30PM)\t1 cup fruit salad",
    "Dinner (8:00-8:30PM)\t1 cup ghia vegetable + 1 chaptti + salad"
]

const list2a = [
    "Breakfast (305 calories): 1 serving Oatmeal with Fresh or Frozen (No Sugar Added) Fruit, 2 tbsp. chopped nuts, 1 cup coffee or tea",
    "A.M. Snack (183 calories): 1 medium banana, 1 tbsp. peanut butter",
    "Lunch (410 calories): Turkey-Avocado Sandwich on whole-grain bread, 1 medium apple",
    "P.M. Snack (119 calories): 8 baby carrots, 2 tbsp. hummus",
    "Dinner (518 calories): 1 serving Salmon with Roasted Vegetables, 1 medium whole-wheat dinner roll, 1 tbsp. butter",
]

const list2b = [
    "Breakfast (266 calories): Avocado-Egg Toast, 1 large banana",
    "A.M. Snack (85 calories): 1 medium orange",
    "Lunch (357 calories): 2 cups Ravioli & Vegetable Soup, 1 small apple",
    "P.M. Snack (169 calories): 6 dried apricots, 12 almonds",
    "Dinner (566 calories): 1 serving Green Salad with Chicken & Strawberries, 2 tbsp. Honey-Mustard Vinaigrette, 1 medium whole-wheat dinner roll, 1 tbsp. butter",
]

const list2c = [
    "Breakfast (274 calories): Peanut Butter-Banana English Muffin, 1 medium orange",
    "A.M. Snack (62 calories): 2 medium celery stalks, 2 tbsp. hummus",
    "Lunch (413 calories): 1 serving Veggie & Hummus Sandwich, 1 small apple",
    "P.M. Snack (120 calories): 1 medium pear, 10 almonds",
    "Dinner (595 calories): 1 serving Pork Chops with Roasted Vegetables, 1 cup cooked brown rice, 1/2 tbsp. olive oil",
]

const list2d = [
    "Breakfast (290 calories): Greek Yogurt with Nectarine and Granola, 1 cup coffee or tea",
    "A.M. Snack (95 calories): 1 medium apple, 1 string cheese",
    "Lunch (412 calories): 1 serving Grilled Cheese with Tomato Soup, 1 medium orange",
    "P.M. Snack (148 calories): 3/4 cup blueberries, 1/4 cup plain Greek yogurt",
    "Dinner (655 calories): 1 serving Spaghetti Squash with Chicken, Pesto, and Roasted Tomatoes, 2 cups mixed greens, 2 tbsp. Italian salad dressing",
]

const list2e =[
    "Breakfast (414 calories): 2 Banana-Pecan Pancakes, 2 Tbsp. maple syrup, 1 medium orange",
    "A.M. Snack (249 calories): 1 hard-boiled egg, 1.5 oz. Cheddar cheese",
    "Lunch (408 calories): 1 serving Chickpea & Quinoa Buddha Bowl, 1 oz. Cheddar cheese",
    "P.M. Snack (305 calories): 1 medium apple, 2 Tbsp. peanut butter",
    "Dinner (624 calories): 1 serving Moroccan-Spiced Chicken Thighs with Sweet Potato Wedges, 2 cups mixed greens, 1/4 cup Carrot-Ginger Vinaigrette",
]

const list2f = [
    "Breakfast (426 calories): 1 serving Scrambled Tofu, 1 slice whole-wheat bread, toasted, 1 medium orange",
    "A.M. Snack (268 calories): 1 medium apple, 2 Tbsp. peanut butter",
    "Lunch (413 calories): 1 serving Chickpea & Quinoa Buddha Bowl, 1 oz. Cheddar cheese",
    "P.M. Snack (292 calories): 1 medium banana, 20 roasted unsalted almonds",
    "Dinner (555 calories): 1 serving Mushroom & Spinach Lasagna, 2 cups mixed greens, 2 Tbsp. Carrot-Ginger Vinaigrette",
]

const list2g = [
    "Breakfast (389 calories): 1 serving Avocado-Egg Toast, 1 medium orange",
    "A.M. Snack (268 calories): 1 medium apple, 2 Tbsp. peanut butter",
    "Lunch (400 calories): 1 serving Chickpea & Quinoa Buddha Bowl, 1 oz. Cheddar cheese",
    "P.M. Snack (288 calories): 1 medium banana, 20 roasted unsalted almonds",
    "Dinner (582 calories): 1 serving Flank Steak with Chimichurri, 1 cup cooked quinoa, 2 cups mixed greens, 2 Tbsp. Carrot-Ginger Vinaigrette",
]

 const list3a = [
    "Breakfast: Aloo parantha, curd, and almonds",
    "Mid-Meal: Pomegranate, guava, and cottage cheese",
    "Lunch: Parboiled rice or 3 chapatis with veg curry, daal, fresh lemon, and salad",
    "Evening: Green tea with honey and 1 cup soybean chaap",
    "Dinner: 2 chapatis with chicken stew and salad",
 ]

const list3b =[
    "Breakfast (8:00-8:30AM) Soy Milk 1 cup + 1 cup Cornflakes + Ripe Banana (2)",
    "Mid-Meal (11:00-11:30AM) 1 Orange + Grapes (1 cup) + 1 cup cottage cheese",
    "Lunch (2:00-2:30PM) Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1/2 cup) + Daal (1/2 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM) Green Tea (1 cup) with 1 tsp honey + 4 Almonds + 5-6 Raisins",
    "Dinner (8:00-8:30PM) 2 Chapati + Veg. Curry (1 cup) + salad",
]

const list3c = [
    "Breakfast (8:00-8:30AM) Veg. Sandwich (2) + Orange juice (1 cup) + 2 boiled eggs",
    "Mid-Meal (11:00-11:30AM) Pomegranate (1 cup) + 1 Guava + 1 cup cottage cheese",
    "Lunch (2:00-2:30PM) Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1 cup) + Daal (1 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM) Green Tea (1 cup) with 1 tsp honey + 4 Almonds + 5-6 Raisins",
    "Dinner (8:00-8:30PM) 2 Chapatti + Chicken stew (1 cup) + salad",
]

const list3d = [
    "Breakfast (8:00-8:30AM) 2 paneer parantha + 1 cup curd + 6 almonds",
    "Mid-Meal (11:00-11:30AM) 1 Orange + Grapes (1 cup) + 1 cup cottage cheese",
    "Lunch (2:00-2:30PM) Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1 cup) + Daal (1 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM) Green Tea (1 cup) with 1 tsp honey + 4 Almonds + 5-6 Raisins",
    "Dinner (8:00-8:30PM) 2 Chapati + Veg. Curry (1 cup) + salad",
]

const list3e = [
    "Breakfast (8:00-8:30AM) Soy Milk 1 cup + 1 cup Cornflakes + Ripe Banana (2)",
    "Mid-Meal (11:00-11:30AM) Pomegranate (1 cup) + 1 Guava + 1 cup cottage cheese",
    "Lunch (2:00-2:30PM) Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1 cup) + Daal (1 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM) Green Tea (1 cup) with 1 tsp honey + 4 Almonds + 5-6 Raisins",
    "Dinner (8:00-8:30PM) 2 Chapatti + Chicken stew (1 cup) + salad",
]

const list3f = [
    "Breakfast (8:00-8:30AM)", "Veg. Sandwich (2) + Orange juice (1 cup) + 2 boiled eggs",
    "Mid-Meal (11:00-11:30AM)", "1 Orange + Grapes (1 cup) + 1 cup cottage cheese",
    "Lunch (2:00-2:30PM)", "Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1 cup) + Daal (1 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM)", "Green Tea (1 cup) with 1 tsp honey + 4 Almonds + 5-6 Raisins",
    "Dinner (8:00-8:30PM)", "2 Chapati + Veg. Curry (1 cup) + salad",
]

const list3g = [
    "Breakfast (8:00-8:30AM)", "2 dal parantha + 1 cup curd + 6 almonds",
    "Mid-Meal (11:00-11:30AM)", "Pomegranate (1 cup) + 1 Guava + 1 cup cottage cheese",
    "Lunch (2:00-2:30PM)", "Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1 cup) + Daal (1 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM)", "Green Tea (1 cup) with 1 tsp honey + 4 Almonds + 5-6 Raisins",
    "Dinner (8:00-8:30PM)", "2 Chapatti + Chicken stew (1 cup) + salad",
]

const list4a =[
    "Breakfast (8:00-8:30AM) 2 aloo parantha + 1 cup curd + 6 almonds",
    "Mid-Meal (11:00-11:30AM) Pomegranate (1 cup) + 1 Guava",
    "Lunch (2:00-2:30PM) Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1/2 cup) + Daal (1/2 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM) Green Tea (1 cup) with 1/2 tsp honey + 1 cup soybean chaap",
    "Dinner (8:00-8:30PM) 2 Chapatti + Chicken stew (1 cup) + salad",
]

const list4b =[
    "Breakfast (8:00-8:30AM) Soy Milk 1 cup + 1 cup Cornflakes + Ripe Banana (1)",
    "Mid-Meal (11:00-11:30AM) 1 Orange + Grapes (1 cup)",
    "Lunch (2:00-2:30PM) Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1/2 cup) + Daal (1/2 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM) Green Tea (1 cup) with 1/2 tsp honey + 4 Almonds + 5-6 Raisins",
    "Dinner (8:00-8:30PM) 2 Chapati + Veg. Curry (1 cup) + salad",
]

const list4c =[
    "Breakfast (8:00-8:30AM) Veg. Sandwich (2) + Orange juice (1 cup)",
    "Mid-Meal (11:00-11:30AM) Pomegranate (1 cup) + 1 Guava",
    "Lunch (2:00-2:30PM) Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1 cup) + Daal (1 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM) Green Tea (1 cup) with 1/2 tsp honey + 4 Almonds + 5-6 Raisins + 4 walnuts",
    "Dinner (8:00-8:30PM) 2 Chapatti + Chicken stew (1 cup) + salad",

]

const list4d =[
    "Breakfast (8:00-8:30AM) 2 paneer parantha + 1 cup curd + 6 almonds",
    "Mid-Meal (11:00-11:30AM) 1 Orange + Grapes (1 cup)",
    "Lunch (2:00-2:30PM) Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1/2 cup) + Daal (1/2 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM) Green Tea (1 cup) with 1/2 tsp honey +1 cup paneer tikka",
    "Dinner (8:00-8:30PM) 2 Chapati + Veg. Curry (1 cup) + salad",
]

const list4e =[
    "Breakfast (8:00-8:30AM)", "Soy Milk 1 cup + 1 cup Cornflakes + Ripe Banana (1)",
    "Mid-Meal (11:00-11:30AM)", "Mixed Nuts (30 gm) + Green Tea (1 cup)",
    "Lunch (1:00-1:30PM)", "Brown Rice (1 katori) + Rajma (1 katori) + Cucumber Raita (1 katori)",
    "Evening Snacks (4:00-4:30PM)", "Chickpea Salad (1 bowl)",
    "Dinner (7:00-7:30PM)", "Roti (1 medium) + Mix Veg (1 katori) + Dal (1 katori)"    
]

const list4f =[
    "Breakfast (8:00-8:30AM) Veg. Sandwich (2) + Orange juice (1 cup)",
    "Mid-Meal (11:00-11:30AM) 1 Orange + Grapes (1 cup)",
    "Lunch (2:00-2:30PM) Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1 cup) + Daal (1 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM) Green Tea (1 cup) with 1/2 tsp honey + 4 Almonds + 5-6 Raisins",
    "Dinner (8:00-8:30PM) 2 Chapati + Veg. Curry (1 cup) + salad",
]

const list4g =[
    "Breakfast (8:00-8:30AM) 2 dal parantha + 1 cup curd + 6 almonds",
    "Mid-Meal (11:00-11:30AM) Pomegranate (1 cup) + 1 Guava",
    "Lunch (2:00-2:30PM) Parboiled Rice (1 cup)/ 3 Chapatis + Veg. curry (1/2 cup) + Daal (1/2 cup) + 1/4th fresh lemon + salad",
    "Evening (4:00-4:30PM) Green Tea (1 cup) with 1/2 tsp honey + 4 Almonds + 5-6 Raisins + 4 walnuts",
    "Dinner (8:00-8:30PM) 2 Chapatti + Chicken stew (1 cup) + salad"   
]


export {list1a,list1b,list1c,list1d,list1e,list1f,list1g,
        list2a,list2b,list2c,list2d,list2e,list2f,list2g,
        list3a,list3b,list3c,list3d,list3e,list3f,list3g,
        list4a,list4b,list4c,list4d,list4e,list4f,list4g,
    }